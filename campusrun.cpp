/*Program: Temple Run 2D (CS335 Final Project)
Authors: Ty Morrell, Ryan Frapwell, John Buenaventura, David Hernandez
Date: Spring 2016
...
bool button2 = false, alien = false, confirmed = false;
bool dave = false;
bool menu = 1;
...
void initOpengl(void)
{
    ...
    //money
    //
    int money_w = moneyImage->width;
    int money_h = moneyImage->height;
    //
    glBindTexture(GL_TEXTURE_2D, moneyTexture);
    //
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, 3, money_w, money_h, 0,
	    GL_RGB, GL_UNSIGNED_BYTE, moneyImage->data);
    //-------------------------------------------------------------------------
    //money silhouette
    //this is similar to a sprite graphic
    //
    ...
    //
    //must build a new set of data...
    unsigned char *MoneysilhouetteData = buildAlphaData(moneyImage);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, money_w, money_h, 0,
	    GL_RGBA, GL_UNSIGNED_BYTE, MoneysilhouetteData);
    //-------------------------------------------------------------------------
    //bat
    //
